...
| Gated session minting | ✅ Done | v0.38.0.0 | `POST /public/candidate/session` requires a `role=user` JWT (401 / 403 for admins); row records `user_id`; idempotent per user so resume survives re-login. Interview calls still trust `X-Anon-Session` alone (accepted, eng review 1+B). |
