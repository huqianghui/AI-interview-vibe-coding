---
spec_issue_number:
spec_filed_at: 2026-08-07T12:48:08Z
spec_branch: main
spec_output: repo SPEC.md (no GitHub issue, per D1)
---
...
**Differentiation vs off-the-shelf (HireVue/Vervoe/etc.):** integrated closed loop + private
control. Runs in the client's own Azure; SOP + candidate data never leave the enterprise; upload
SOP → AI-generate question bank + rubric → digital-human interview → report, one loop. Incumbents
do generic behavioral/skills assessment as hosted SaaS and cover only one segment.
